...
export default {
    ...
    methods: {
        setData(data) {
            ...
            this.$refs.form.getOptionsFromParent('publication',
                [{"text": data.publication_data.title,
                  "value": data.publication_data.id}])
        },
        getItem() {
            let source = '/api/editorial-board/newsletters/'+this.newsletter_id+'/messages/'+this.message_id+'/publications/'+this.publication_id+'/';
            this.axios
                .get(source)
                .then(response => {
        ...
        remove() {
            this.axios
                .delete('/api/editorial-board/newsletters/'+this.newsletter_id+'/messages/'+this.message_id+'/publications/'+this.publication_id+'/',
                        {headers: {"X-CSRFToken": this.$csrftoken }}
                       )
                .then(response => {
                    this.alerts.push(
