                ...
                <div class="col-12">
                    <b-card>
                        <div class="clearfix mb-5">
                            <div class="pull-left">
                                <router-link :to="{ name: 'WebpathPageContactLogs',
...
export default {
    data() {
        return {
            ...
            form: {},
            form_source: '/api/editorial-board/sites/'+this.$route.params.site_id+'/webpaths/'+this.$route.params.webpath_id+'/pages/'+this.$route.params.page_id+'/contacts/form/',
            add_modal_fields: {'contact':  this.$router.resolve({name: 'ContactNew'}).href},
            page_title: '',
            redis_alert: null,
            interval: null,
        }
    ...
    methods: {
        ...
        onSubmit(event) {
            ...
            this.axios
                ...
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'page contact edited successfully',
                          dismissable: true }
                    );
        ...
        remove() {
            this.axios
                .delete('/api/editorial-board/sites/'+this.site_id+'/webpaths/'+this.webpath_id+'/pages/'+this.page_id+'/contacts/'+this.contact_id+'/',
                        {headers: {"X-CSRFToken": this.$csrftoken }}
                       )
                .then(response => {
                    this.page_title = response.data.contact.name;
                    this.alerts.push(
                        { variant: 'success',
                          message: 'page contact removed successfully',
                          dismissable: true }
                    );
                    this.$router.push({name: 'WebpathPageContacts',
        ...
        deleteModal() {
            this.$bvModal.msgBoxConfirm('Do you want really delete page contact?', {
