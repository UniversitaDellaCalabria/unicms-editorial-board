...
export default {
    data() {
        return {
            ...
            form: {},
            form_source: '/api/editorial-board/newsletters/'+this.$route.params.newsletter_id+'/test-subscriptions/form/',
            files: {},
            page_title: '',
            redis_alert: null,
            interval: null,
    ...
    methods: {
        ...
        getItem() {
            let source = '/api/editorial-board/newsletters/'+this.newsletter_id+'/test-subscriptions/'+this.subscription_id+'/';
            this.axios
                .get(source)
                .then(response => {
        ...
        onSubmit(event) {
            ...
            this.axios
                ...
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'newsletter test subscription edited successfully',
                          dismissable: true }
                    )
        ...
        remove() {
            this.axios
                .delete('/api/editorial-board/newsletters/'+this.newsletter_id+'/test-subscriptions/'+this.subscription_id+'/',
                        {headers: {"X-CSRFToken": this.$csrftoken }}
                       )
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'newsletter test subscription removed successfully',
                          dismissable: true }
                    );
                    this.$router.push({name: 'NewsletterTestSubscriptions',
        ...
        deleteModal() {
            this.$bvModal.msgBoxConfirm('Do you want really delete test subscription?', {
