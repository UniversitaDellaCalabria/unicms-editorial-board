                ...
                <div class="col-12">
                    <b-card>
                        <div class="clearfix mb-5">
                            <div class="pull-left">
                                <router-link :to="{ name: 'WebpathCalendarLogs',
...
export default {
    data() {
        return {
            ...
            form: {},
            form_source: '/api/editorial-board/sites/'+this.$route.params.site_id+'/webpaths/calendar-contexts/form/',
            page_title: '',
            redis_alert: null,
            interval: null,
        }
    ...
    methods: {
        setData(data) {
            ...
            this.page_title = data.calendar.name;
            this.$refs.form.getOptionsFromParent('webpath',
                [{"text": data.webpath.full_name,
                  "value": data.webpath.id}])
            this.$refs.form.getOptionsFromParent('calendar',
                [{"text": data.calendar.name,
                  "value": data.calendar.id}])
        },
        getItem() {
            let source = '/api/editorial-board/sites/'+this.site_id+'/webpaths/'+this.webpath_id+'/calendar-contexts/'+this.calendar_id+'/';
            this.axios
                .get(source)
                .then(response => {
        ...
        onSubmit(event) {
            ...
            let source = '/api/editorial-board/sites/'+this.site_id+'/webpaths/'+this.webpath_id+'/calendar-contexts/'+this.publication_id+'/';
            ...
            this.axios
                ...
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'webpath calendar edited successfully',
                          dismissable: true }
                    );
                    ...
                    this.$router.push({name: 'WebpathPublicationEdit',
                                       params: {site_id: this.site_id,
                                                ...
                                                alerts: this.alerts}})
                                .catch(() => {})
                    this.page_title = response.data.calendar.name;
        ...
        remove() {
            this.axios
                .delete('/api/editorial-board/sites/'+this.site_id+'/webpaths/'+this.webpath_id+'/calendar-contexts/'+this.publication_id+'/',
                        {headers: {"X-CSRFToken": this.$csrftoken }}
                       )
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'webpath calendar removed successfully',
                          dismissable: true }
                    );
        ...
        deleteModal() {
            this.$bvModal.msgBoxConfirm('Do you want really delete webpath calendar?', {
