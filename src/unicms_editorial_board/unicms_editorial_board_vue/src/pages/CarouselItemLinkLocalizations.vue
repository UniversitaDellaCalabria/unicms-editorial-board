<template>
    <div class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <b-card title="Carousel item link localizations">
                        <b-card-text>
                                ...
                                <template #cell(actions)="data">
                                    <router-link :to="{ name: 'CarouselItemLinkLocalizationEdit',
                                                    params: { carousel_id: carousel_id,
                                                              carousel_item_id: carousel_item_id,
                                                              carousel_item_link_id: carousel_item_link_id,
                                                              carousel_item_link_localization_id: data.item.id}}"
...
export default {
    ...
    methods: {
        callApi(url, page=null) {
            ...
            let source = '/api/editorial-board/carousels/'+this.carousel_id+'/items/'+this.carousel_item_id+'/links/'+this.carousel_item_link_id+'/localizations/?page=' + target_page + '&search=' + this.search;
            if (url) source = url;
            this.axios
                .get(source)
                .then(response => {
        ...
        changeStatus(id) {
            ...
            this.axios
                .patch('/api/editorial-board/carousels/'+this.carousel_id+'/items/'+this.carousel_item_id+'/links/'+this.carousel_item_link_id+'/localizations/'+item.id+'/',
        ...
        remove(id) {
            this.axios
                .delete('/api/editorial-board/carousels/'+this.carousel_id+'/items/'+this.carousel_item_id+'/links/'+this.carousel_item_link_id+'/localizations/'+id+'/',
                        {headers: {"X-CSRFToken": this.$csrftoken }}
                       )
                .then(response => {
        ...
        deleteModal(item) {
            this.$bvModal.msgBoxConfirm('Do you want really delete carousel item link localization '+item.title+'?', {
