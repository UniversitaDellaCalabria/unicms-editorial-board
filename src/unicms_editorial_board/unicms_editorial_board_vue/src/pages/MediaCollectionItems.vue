                            ...
                            <b-table
                                ...
                                <template #cell(actions)="data">
                                    <router-link :to="{ name: 'MediaCollectionItemEdit',
                                                    params: { collection_id: data.item.collection,
                                                              collection_item_id: data.item.id}}"
...
export default {
    data () {
        return {
            ...
            fields: [
                {key: 'media', sortable: true, thClass: 'w-25'},
    ...
    methods: {
        callApi(url, page=null) {
            ...
            let source = '/api/editorial-board/media-collections/'+this.collection_id+'/items/?page=' + target_page + '&search=' + this.search;
            if (url) source = url;
            this.axios
                .get(source)
                .then(response => {
        ...
        changeStatus(id) {
            ...
            this.axios
                .patch('/api/editorial-board/media-collections/'+this.collection_id+'/items/'+item.id+'/',
        ...
        remove(id) {
            this.axios
                .delete('/api/editorial-board/media-collections/'+this.collection_id+'/items/'+id+'/',
                        {headers: {"X-CSRFToken": this.$csrftoken }}
                       )
                .then(response => {
        ...
        deleteModal(item) {
            this.$bvModal.msgBoxConfirm('Do you want really delete collection item?', {
