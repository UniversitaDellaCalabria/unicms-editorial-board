...
export default {
    ...
    methods: {
        onSubmit(event) {
            let source = '/api/editorial-board/sites/'+this.site_id+'/webpaths/'+this.webpath_id+'/pages/'+this.page_id+'/headings/'+this.$route.params.heading_id+'/localizations/';
            ...
            this.axios
                ...
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'page heading localization added successfully',
                          dismissable: true }
                    );
