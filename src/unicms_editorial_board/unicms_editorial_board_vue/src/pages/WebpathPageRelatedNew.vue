...
export default {
    ...
    methods: {
        onSubmit(event) {
            let source = '/api/editorial-board/sites/'+this.site_id+'/webpaths/'+this.webpath_id+'/pages/'+this.page_id+'/related/';
            ...
            this.axios
                ...
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'page related added successfully',
                          dismissable: true }
                    );
                    this.$router.push({name: 'WebpathPageRelated',
