...
export default {
    ...
    methods: {
        ...
        getItem() {
            let source = '/api/editorial-board/newsletters/'+this.newsletter_id+'/subscriptions/'+this.subscription_id+'/';
            this.axios
                .get(source)
                .then(response => {
        ...
        onSubmit(event) {
            ...
            this.axios
                ...
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'newsletter subscription edited successfully',
                          dismissable: true }
                    )
        ...
        remove() {
            this.axios
                .delete('/api/editorial-board/newsletters/'+this.newsletter_id+'/subscriptions/'+this.subscription_id+'/',
                        {headers: {"X-CSRFToken": this.$csrftoken }}
                       )
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'newsletter subscription removed successfully',
                          dismissable: true }
                    );
        ...
        deleteModal() {
            this.$bvModal.msgBoxConfirm('Do you want really delete subscription?', {
