...
export default {
    data() {
        return {
            ...
            form: {},
            form_source: '/api/editorial-board/newsletters/'+this.$route.params.newsletter_id+'/messages/'+this.$route.params.message_id+'/webpaths/form/',
            files: {},
            page_title: '',
            redis_alert: null,
            interval: null,
    ...
    methods: {
        setData(data) {
            ...
            this.$refs.form.getOptionsFromParent('webpath',
                [{"text": data.webpath_data,
                  "value": data.webpath }])
        },
        getItem() {
            let source = '/api/editorial-board/newsletters/'+this.newsletter_id+'/messages/'+this.message_id+'/webpaths/'+this.webpath_id+'/';
            this.axios
                .get(source)
                .then(response => {
        ...
        onSubmit(event) {
            ...
            this.axios
                ...
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'webpath edited successfully',
                          dismissable: true }
                    )
        ...
        remove() {
            this.axios
                .delete('/api/editorial-board/newsletters/'+this.newsletter_id+'/messages/'+this.message_id+'/webpaths/'+this.webpath_id+'/',
                        {headers: {"X-CSRFToken": this.$csrftoken }}
                       )
                .then(response => {
                    this.alerts.push(
                    ...
                    this.$router.push({name: 'NewsletterMessageWebpaths',
