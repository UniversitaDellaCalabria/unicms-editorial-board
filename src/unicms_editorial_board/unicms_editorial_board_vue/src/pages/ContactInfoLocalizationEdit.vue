                ...
                <div class="col-12">
                    <b-card>
                        <div class="clearfix mb-5">
                            <div class="pull-left">
                                <router-link :to="{ name: 'ContactInfoLocalizationLogs',
...
export default {
    ...
    methods: {
        ...
        getItem() {
            let source = '/api/editorial-board/contacts/'+this.$route.params.contact_id+'/infos/'+this.$route.params.contact_info_id+'/localizations/'+this.localization_id+'/';
            this.axios
                .get(source)
                .then(response => {
        ...
        onSubmit(event) {
            ...
            this.axios
                ...
                .then(response => {
                    this.page_title = response.data.label;
                    this.alerts.push(
                        { variant: 'success',
                          message: 'contact information localization edited successfully',
                          dismissable: true }
                    );
        ...
        remove() {
            this.axios
                .delete('/api/editorial-board/contacts/'+this.contact_id+'/infos/'+this.contact_info_id+'/localizations/'+this.localization_id+'/',
                        {headers: {"X-CSRFToken": this.$csrftoken }}
                       )
                .then(response => {
                    this.alerts.push(
                        { variant: 'success',
                          message: 'contact information localization removed successfully',
                          dismissable: true }
                    );
        ...
        deleteModal() {
            this.$bvModal.msgBoxConfirm('Do you want really delete contact information localization?', {
